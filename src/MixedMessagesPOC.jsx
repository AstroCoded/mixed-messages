...
const FILLER = [
  "SUNBEAM","BREEZE","STARDUST","TWINKLE","GIGGLE","CONFETTI","BUBBLE","FEATHER","MEADOW","SKYLARK",
  "RAINBOW","MOONBEAM","WHIMSY","WONDER","CHEER","BRIGHT","PLAYFUL","UPLIFT","LILT","LIGHTNESS",
  "MAGIC","MARVEL","SMILE","BLOSSOM","HEARTEN","ZENITH","ZEST","GLOW","GLIMMER","SHIMMER",
  "SERENE","BREEZY","SUNLIT","HOPE","KINDNESS","GRACE","HARMONY","LUCKY","SPIRIT","BOLD",
  "BRIGHTEN","SOAR","LIFT","RADIANT","TRUST","PEACE","UNITY","CALM","JOYFUL","SPARKLE",
  "COURAGE","CURIOUS","DREAM","KIND","MIRTH","CHEERY","HUG","RIPPLE","SING","SWAY",
];

// Pastel color palette for fade-in
const PASTEL_COLORS = [
  "#F8BBD0", "#E1BEE7", "#BBDEFB", "#B2EBF2", "#C8E6C9", "#FFF9C4", "#FFCCBC", "#D7CCC8", "#F0F4C3", "#DCEDC8",
  "#FFE0B2", "#D1C4E9", "#B3E5FC", "#C5CAE9", "#FFECB3", "#F48FB1", "#80CBC4", "#A5D6A7", "#FFAB91", "#B39DDB"
];
...
export default function MixedMessagesPOC() {
  ...
  const [order, setOrder] = useState(() => shuffle([...Array(PHRASES.length).keys()]));
  const [orderPos, setOrderPos] = useState(0);
  const [phraseIdx, setPhraseIdx] = useState(() => shuffle([...Array(PHRASES.length).keys()])[0]);
  ...
  const { gridWords, wordIndex } = useMemo(() => {
    ...
    while (wordsForGrid.length < cellCount) {
      ...
      wordsForGrid.push(fillerPool[Math.floor(Math.random() * fillerPool.length)]);
    }
  ...
  const allPhraseWords = useMemo(() => PHRASES.map((p) => p.toUpperCase().split(/\s+/)), []);
  ...
  useEffect(() => {
    function startSequence() {
      ...
      const totalTime = (indices.length - 1) * nextStartOffset + fadeOverlapStart + FADE_MS + PHRASE_PAUSE_MS;
  ...
  return (
    <div className="min-h-screen w-full text-zinc-200 flex flex-col items-center py-10" style={{ backgroundColor: "#0f0f10" }}>
      <div className="w-full max-w-5xl px-4 mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold tracking-tight">Mixed Messages — Web POC</h1>
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="w-40"
          />
          <div className="w-10 text-right tabular-nums text-sm">{speed.toFixed(1)}×</div>
        </div>
      </div>

      <div className="rounded-3xl shadow-2xl p-6" style={{ background: "#0b0b0c", border: "1px solid #1f2024" }}>
        <div
          className="grid"
          style={{
            display: "grid",
        ...
        >
          {gridWords.map((word, i) => {
            ...
            return (
              <div key={i} className="relative flex items-center justify-center" style={{ transition: `all ${FADE_MS}ms ease` }}>
                <div
